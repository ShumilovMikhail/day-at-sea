...
import { InfrastructureItemVM, InfrastructureListVM } from '../../types/infrastructure.models';
...
export class AddObjectInfrastructureListUiComponent {
  @Input({ required: true }) infrastructureList!: InfrastructureListVM;
  ...
  public findCheckboxControl(type: string, name: string): InfrastructureItemVM | null {
    const control = (this.infrastructureList[type as keyof InfrastructureListVM] as FormArray).controls.find(
